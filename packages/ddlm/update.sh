...
CURRENT_REV=$(grep 'rev = ' "$DEFAULT_NIX" | head -1 | sed 's/.*"\([^"]*\)".*/\1/')
...
sed -i "s|rev = \"[^\"]*\"|rev = \"$LATEST_COMMIT\"|" "$DEFAULT_NIX"
sed -i "s|sha256 = \"sha256-[^\"]*\"|sha256 = \"$SOURCE_HASH_SRI\"|" "$DEFAULT_NIX"
...
if nix-build -E "with import <nixpkgs> {}; callPackage $DEFAULT_NIX {}" >/dev/null 2>&1; then
    ...
    echo "Updated ddlm from $CURRENT_REV to $LATEST_COMMIT"
else
