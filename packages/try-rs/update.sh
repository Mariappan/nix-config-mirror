...
SOURCE_HASH=$(nix-prefetch-url --unpack "https://github.com/$OWNER/$REPO/archive/refs/tags/v${LATEST_VERSION}.tar.gz" 2>/dev/null)
...
if nix-build -E "with import <nixpkgs> {}; callPackage $DEFAULT_NIX {}" >/dev/null 2>&1; then
    ...
    echo "Updated try-rs from $CURRENT_VERSION to $LATEST_VERSION"
else
